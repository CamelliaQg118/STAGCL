...
class BuildPositivePair():
    ...
    def cal_positive_emb(self, emb, I_knn, I_kmeans, centroids):
        ...
        spot_candi_emb = emb[I_knn.reshape(-1, 1).squeeze().numpy(), :].reshape(
            (-1, I_knn.shape[1], emb.shape[1]))  # get candidate positive samples emb
    ...
    def fit(self, z, top_k):
        ...
        _, I_knn = similarity.topk(k=top_k, dim=1, largest=True,
                                   sorted=True)  # knn-->construct knn candidate positive samples
